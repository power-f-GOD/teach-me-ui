...
it("profile edit reducers should be called with 'state' and 'action' params and return value of initial state type.", () => {
  ...
  const getUserDetailsMockFunc = jest.fn(
    (state: EditProfileState, action: ReduxAction) =>
      reducers.getUserDetails(state, action)
  );

  const updateAcademicDataMockFunc = jest.fn(
    (state: EditProfileState, action: ReduxAction) =>
      reducers.updateAcademicData(state, action)
  );

  const updateEmailMockFunc = jest.fn(
    (state: EditProfileState, action: ReduxAction) =>
      reducers.updateEmail(state, action)
  );

  const updatePasswordMockFunc = jest.fn(
    (state: EditProfileState, action: ReduxAction) =>
      reducers.updatePassword(state, action)
  );

  const updateUserDataMockFunc = jest.fn(
    (state: EditProfileState, action: ReduxAction) =>
      reducers.updateUserData(state, action)
  );

  const updateUsernameMockFunc = jest.fn(
    (state: EditProfileState, action: ReduxAction) =>
  ...
  expect(getUserDetailsMockFunc).toHaveBeenCalledWith(
    editProfileState,
    getUserDetailsRequestAction
  );
  expect(updateAcademicDataMockFunc).toHaveBeenCalledWith(
    editProfileState,
    updateAcademicDataRequestAction
  );
  expect(updateEmailMockFunc).toHaveBeenCalledWith(
    editProfileState,
    updateEmailRequestAction
  );
  expect(updatePasswordMockFunc).toHaveBeenCalledWith(
    editProfileState,
    updatePasswordRequestAction
  );
  expect(updateUserDataMockFunc).toHaveBeenCalledWith(
    editProfileState,
    updateUserDataRequestAction
  );
  expect(updateUsernameMockFunc).toHaveBeenCalledWith(
    editProfileState,
    updateUsernameRequestAction
  );
  expect(
    reducers.getUserDetails(editProfileState, getUserDetailsRequestAction)
  ).toMatchObject(mockEditProfileState);
  expect(
    reducers.updateAcademicData(
      editProfileState,
      updateAcademicDataRequestAction
    )
  ).toMatchObject(mockEditProfileState);
  expect(
    reducers.updateEmail(editProfileState, updateEmailRequestAction)
  ).toMatchObject(mockEditProfileState);
  expect(
    reducers.updatePassword(editProfileState, updatePasswordRequestAction)
  ).toMatchObject(mockEditProfileState);
  expect(
    reducers.updateUserData(editProfileState, updateUserDataRequestAction)
  ).toMatchObject(mockEditProfileState);
  expect(
    reducers.updateUsername(editProfileState, updateUsernameRequestAction)
  ).toMatchObject(mockEditProfileState);
});
