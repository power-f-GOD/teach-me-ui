...
it("sends data to the server to update a user's profile", () => {
  ...
  const updatePasswordRequestMockFunc = jest.fn((current_password: string, new_password: string) => {
    return (dispatch: Function) => {
      actions.getUserDetails(mockEditProfileState);
    }
  });
  ...
  const updateAcademicDataRequestMockFunc = jest.fn((level: string, department: string) => {
    return (dispatch: Function) => {
      actions.getUserDetails(mockEditProfileState);
    }
  });
  ...
  expect(updatePasswordRequestMockFunc).toHaveBeenCalledWith(mockCurrentPassword, mockNewPassword);
  ...
  expect(updateAcademicDataRequestMockFunc).toHaveBeenCalledWith(mocklevel, mockDepartment);
  expect(actions.getUserDetails(mockEditProfileState)).toMatchObject(getUserDetailsAction);
  expect(actions.updateAcademicData(mockEditProfileState)).toMatchObject(updateAcademicDataAction);
  expect(actions.updateEmail(mockEditProfileState)).toMatchObject(updateEmailAction);
  expect(actions.updatePassword(mockEditProfileState)).toMatchObject(updatePasswordAction);
  expect(actions.updateUsername(mockEditProfileState)).toMatchObject(updateUsernameAction);
  expect(actions.updateUserData(mockEditProfileState)).toMatchObject(updateUserDataAction);
});