    ...
    <Box p={1} pt={0}>
      {sendFiles.status === 'pending' ? (
        <Row className='d-flex mx-auto mt-1'>
          <Box className='bold' component='p'>
      ...
      <Row
        as='h4'
        className={`${
          uploads.data[0] ? 'display-block' : 'display-none'
        } upload-select-header margin-top-1`}>
        {' '}
        Uploads
      </Row>
      <Row className='d-flex mx-auto mt-1'>
        <Box component='div' id='upload-grid-box1' className='scroll-image'>
...
const mapStateToProps = ({ sendFiles, uploads }: any) => ({
  sendFiles,
  uploads
});
